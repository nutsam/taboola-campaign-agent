...
class MigrationReport:
    ...
    def __str__(self):
        # Using a single, clean, triple-quoted f-string to avoid syntax errors.
        return f"""
--- Migration Report ---
...
class PlatformAdapter(ABC):
    ...
    def map_to_taboola(self, source_data: dict, schema: dict) -> tuple[dict, list]:
        """Maps source data to Taboola fields using a predefined schema."""
        logging.info(f"Mapping {self.__class__.__name__} fields to Taboola fields using schema...")
        taboola_campaign = {}
        warnings = []

        for taboola_field, mapping_rules in schema.items():
...
class FacebookAdapter(PlatformAdapter):
    ...
    SCHEMA = {
        'name': {'source_field': 'name', 'default': 'Migrated Campaign'},
        'daily_cap': {
            'source_field': 'daily_budget', 
            'default': 20.0, 
            'transform': lambda x: x / 100 if x is not None else 20.0
        },
        'branding_text': {
            'source_field': 'name', 
            'default': 'Your Brand', 
            'warning': "Used Facebook campaign name as 'branding_text'. Please review in Taboola."
        },
        'cpc_bid': {'default': 0.50, 'warning': "Set a default CPC bid of $0.50. Please adjust."},
        'creatives': {
            'source_field': 'creatives', 
            'default': [], 
            'transform': lambda creatives: [{'photo_url': c.get('image_url'), 'title': c.get('headline')} for c in creatives or []]
        }
    }
    ...
    def map_to_taboola(self, fb_data: dict) -> tuple[dict, list]:
        return super().map_to_taboola(fb_data, self.SCHEMA)

class TwitterAdapter(PlatformAdapter):
    """Adapter for migrating campaigns from Twitter."""
    SCHEMA = {
        'name': {'source_field': 'name', 'default': 'Migrated Twitter Campaign'},
        'daily_cap': {'source_field': 'total_budget', 'default': 25.0},
        'branding_text': {'source_field': 'account_name', 'default': 'Your Brand'},
        'cpc_bid': {'default': 0.60, 'warning': "Set a default CPC bid of $0.60 for Twitter. Please adjust."},
        'creatives': {
            'source_field': 'tweet_creatives', 
            'default': [],
            'transform': lambda creatives: [{'photo_url': c.get('media_url'), 'title': c.get('text')} for c in creatives or []]
        }
    }
    ...
    def map_to_taboola(self, twitter_data: dict) -> tuple[dict, list]:
        return super().map_to_taboola(twitter_data, self.SCHEMA)
...
class MigrationModule:
    ...
    def __init__(self, taboola_client: TaboolaApiClient, source_clients: dict):
        ...
        self.adapters = {
            ...
            'twitter': TwitterAdapter(source_clients.get('twitter')) # Assuming a client is provided
        }
    ...
    def migrate_campaign(self, source_platform: str, campaign_id: str, data_override: dict = None) -> MigrationReport:
        ...
        try:
            ...
            if data_override:
                # Remove keys where the override value is None
